...
def compute_center(lines):
    ...
    * punto medio entre ambos bordes. Se requieren ambos lados para que el resultado
    * sea válido: si solo se ve un borde, el PID no actúa y el auto conserva el
    * rumbo anterior. Esto evita que las rayas diagonales de los cruces peatonales,
    * que tienden a inclinarse todas hacia el mismo lado, confundan al volante.
    """
    if lines is None:
        return None
    lx, rx = [], []
    ...
    return None   # sin ambos bordes no hay centro confiable
    ...
    for scale in (4.0,):
        ...
        for y in range(0, sh - HOG_WIN_H + 1, ystep):
            for x in range(x0, min(x1, sw - HOG_WIN_W + 1), SLIDE_STEP):
                ...
                # * Si la ventana tiene más de 15% de píxeles amarillos es una
                # * línea del carril o un cruce peatonal, no una persona. La
                # * saltamos sin correr el análisis para ahorrar tiempo de cómputo.
...
def main():
    ...
    prev_valid_center          = None   # último centro válido aceptado por el PID
    frame_cnt  = 0
    ...
    while driver.step() != -1:
        ...
        # * Para saber a dónde girar el volante, el auto filtra la imagen y se queda
        # * solo con los píxeles amarillos del carril. Detecta los bordes de esa
        # * máscara, recorta la zona trapezoidal del carril, busca segmentos de línea
        # * y calcula el punto medio entre las líneas de la izquierda y la derecha.
        # * Ese punto medio es el objetivo que el PID intenta mantener centrado.
        ...
        # * Si el centro detectado salta más del 30% del ancho de pantalla respecto
        # * al frame anterior, lo descartamos como detección falsa. Las líneas reales
        # * del carril se mueven suavemente; un salto brusco indica una marca de cruce
        # * peatonal o una línea de la calle perpendicular que se coló en la imagen.
        # * Después de 30 frames sin línea válida reseteamos la referencia para que
        # * la primera detección al salir de la intersección no sea rechazada.
        if center is not None:
            if prev_valid_center is not None and abs(center - prev_valid_center) > dw * 0.30:
                center = None   # salto brusco → falsa detección
            else:
                prev_valid_center = center
        if no_line_frames > 30:
            prev_valid_center = None   # reset tras intersección larga
        ...
        # * la cámara quien vio a la persona. El ángulo del volante se conserva
        # * para que el auto retome la curva correctamente al reanudar la marcha.
        if threat != 'none':
        ...
        if center is not None:
            ...
            # * Cuando no hay línea amarilla visible, como en una intersección o
            # * un cruce peatonal, el auto mantiene el ángulo anterior durante
            # * 8 frames para conservar la dirección que traía. Ese tiempo es
            # * suficiente para cruzar una cebra angosta sin perder el rumbo.
            # * Pasados esos 8 frames el volante vuelve al centro más rápido
            # * que antes porque a 30 km/h la intersección dura más frames y
            # * un decay lento acumula demasiado desvío antes de ver las líneas.
            no_line_frames += 1
            ...
            if no_line_frames > 20:
                steering *= 0.97     # decay suave: 0.97^20 ≈ 0.54 a los 20 frames
