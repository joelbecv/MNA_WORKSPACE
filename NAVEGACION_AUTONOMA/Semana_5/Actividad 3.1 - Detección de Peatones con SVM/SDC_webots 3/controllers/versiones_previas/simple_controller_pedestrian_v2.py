...
HOLD_FRAMES   = 80    # frames mínimos de freno activo (~0.8 s a 10 ms/frame)
...
def svm_detect(bgr, model):
    ...
    """
    h, w      = bgr.shape[:2]
    y_off     = int(h * 0.51)   # −10% espacio horizontal (29px→26px)
    roi       = bgr[y_off : int(h * 0.71), :]
    total     = 0
    max_score = -999.0
    windows   = 0
    best_box  = None

    for scale in (5.0,):        # ROI 29px: scale 4.0 requiere 32px mínimo, se usa 5.0
        scaled = cv2.resize(roi, (int(w*scale), int(roi.shape[0]*scale)))
        sh, sw = scaled.shape[:2]
        if sh < HOG_WIN_H or sw < HOG_WIN_W:
            continue
        x0        = int(sw * 0.30)   # −10% ancho (44%→40%)
        x1        = int(sw * 0.70)
...
def main():
    ...
    while driver.step() != -1:
        ...
        # Solo bordes amarillos — el Canny sobre grey capturaba siluetas de
        # peatones y marcas de cruce que competían con las líneas del carril
