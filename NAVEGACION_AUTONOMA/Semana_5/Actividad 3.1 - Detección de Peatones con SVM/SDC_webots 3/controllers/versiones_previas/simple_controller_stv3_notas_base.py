...
MIN_ABS_SLOPE  = 0.4      # pendiente mínima aceptada (0=horizontal, 1=45°)
...
def compute_center(lines):
    ...
    * punto medio entre ambos bordes. Si solo se ve uno, se usa ese como referencia
    * para no perder el control cuando el carril no es completamente visible.
    """
    if lines is None:
        return None
    lx, rx, ax = [], [], []
...
def main():
    ...
    while driver.step() != -1:
        ...
        if center is not None:
            ...
            # * 20 frames para conservar la dirección que traía. Ese tiempo es
            # * suficiente para cruzar una cebra angosta sin perder el rumbo.
            # * Pasados esos 20 frames el volante vuelve gradualmente al centro
            # * con un decay del 3% por frame, para no acumular demasiado desvío
            # * antes de que aparezcan de nuevo las líneas del carril.
            no_line_frames += 1
