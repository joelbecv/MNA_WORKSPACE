...
HOLD_FRAMES   = 230   # frames mínimos de freno activo (30 base + 200 = ~2 s a 10 ms/frame)
...
def svm_detect(bgr, model):
    ...
    """
    h, w      = bgr.shape[:2]
    y_off     = int(h * 0.55)   # −10% alto: sube el techo del ROI
    roi       = bgr[y_off : int(h * 0.83), :]
    total     = 0
    max_score = -999.0
    windows   = 0
    best_box  = None
    ...
    for scale in (4.0,):        # ROI 36px: scale 3.0 requiere 43px mínimo, se usa 4.0
        ...
        if sh < HOG_WIN_H or sw < HOG_WIN_W:
            continue
        x0        = int(sw * 0.18)   # −10% ancho: márgenes laterales más ajustados
        x1        = int(sw * 0.82)
